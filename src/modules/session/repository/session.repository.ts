...
import { Session, SessionAnswer, SessionMarker } from '../domain/session';
...
export class SessionRepository {
    ...
    async upsert(uow: UnitOfWork, session: Session): Promise<void> {
        await uow.tx
            .insert(sessions)
            .values(session)
    ...
    //     const existingAnswers = await uow.tx
    //         .select()
    //         .from(sessionAnswers)
    //         .where(eq(sessionAnswers.sessionId, session.id));
    //     const existingMarkers = await uow.tx
    //         .select()
    //         .from(sessionMarkers)
    ...
    //         const found = existingMarkers.find(
